// Typed error model usecases return instead of bare errors. The delivery layer maps
// *AppError → HTTP status via respondError (response.go), eliminating the scattered
// strings.Contains(err.Error(), "not found") checks in the current handlers.
package domain
